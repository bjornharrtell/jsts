...
// Remove https://github.com/bjornharrtell/javascript.util/tree/master/src
/**
 ...
 */
//jsts.operation.valid.IsValidOp.isValid = function(geom) {
// TODO Javaklassen isValid(Geometry)och instantsmetod sValid(). Funkar nog inte såhär...
//  var isValidOp = new jsts.operation.valid.IsValidOp(geom);
...
jsts.operation.valid.IsValidOp.isValid = function(arg) {
  // jsts.operation.valid.IsValidOp.isValidCoord = function(coord) {
  if (arguments[0] instanceof jsts.geom.Coordinate) {
    ...
    if (!isFinite(arg.y) && !isNaN(arg.y)) { // TODO: AAHA
      return false;
    }
    return true;
  } else {
    var isValidOp = new jsts.operation.valid.IsValidOp(arg); // AAHA Replace jsts.operation.valid.IsValidOp.isValid = function(geom)
    return isValidOp.isValid();
  }
...
jsts.operation.valid.IsValidOp.prototype.setSelfTouchingRingFormingHoleValid = function(
    isValid) {
  this.isSelfTouchingRingFormingHoleValid = isValid; // AAHA
};
...
jsts.operation.valid.IsValidOp.prototype.checkValidPoint = function(g) {
  this.checkInvalidCoordinates(g.getCoordinates());// AAHA
};
/**
...
jsts.operation.valid.IsValidOp.prototype.checkValidLineString = function(g) {
  ...
  this.checkTooFewPoints(graph);// AAHA
};
/**
...
jsts.operation.valid.IsValidOp.prototype.checkValidPolygon = function(g) {
  this.checkInvalidCoordinates(g); // AAHA
  if (this.validErr != null) {
    return;
  }
  this.checkClosedRings(g); // AAHA
  if (this.validErr != null) {
    return;
  }
