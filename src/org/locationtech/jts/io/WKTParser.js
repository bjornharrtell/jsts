...
export default class WKTParser {
  ...
  get extract() { return {
    ...
    'linestring': function(linestring) {
      var array = [];
      for ( var i = 0, len = linestring.points.coordinates.length; i < len; ++i) {
        array.push(this.extract['coordinate'].apply(this, [linestring.points.coordinates[i]]));
      }
      return array.join(',');
    },
  ...
  get parse() { return {
    ...
    'geometrycollection': function(str) {
      ...
      for ( var i = 0, len = wktArray.length; i < len; ++i) {
        components.push(WKTParser.prototype.read.apply(this,
            [wktArray[i]]));
      }
