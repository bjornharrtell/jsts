...
declare namespace jsts {
  ...
  export class JstsIterator<T> {
    hasNext(): boolean;
    next(): T;
  }
  export class JstsCollection<T> {
    iterator(): JstsIterator<T>;
  }
  ...
  namespace triangulate {
    import Geometry = jsts.geom.Geometry;
    import GeometryCollection = jsts.geom.GeometryCollection;
    import GeometryFactory = jsts.geom.GeometryFactory;

    export class DelaunayTriangulationBuilder {
      setSites(sites: Geometry | GeometryCollection): void;
      getTriangles(gf: GeometryFactory): Geometry;
    }
  }
  ...
  namespace geomgraph {
    ...
    import LineIntersector = jsts.algorithm.LineIntersector;
    import SegmentIntersector = index.SegmentIntersector;
    ...
    export class EdgeIntersectionList {
      iterator(): JstsIterator<EdgeIntersection>;
    }

    export class Edge {
      getCoordinate(idx: number): Coordinate;
  ...
  namespace geom {
    export class CoordinateList {
      constructor(cood: geom.Coordinate[]);
      constructor(cood: geom.Coordinate[], allowRepeated: boolean);
      getCoordinate(i: number): geom.Coordinate;
      toCoordinateArray(): geom.Coordinate[];
    }
    ...
    export class GeometryFactory {
      ...
      // Probably update to use polygons to be more strictly correct.
      createMultiPolygon(geometry: Geometry[]): MultiPolygon;
      /**
  ...
  namespace operation {
    ...
    namespace polygonize {
      export class Polygonizer {
        ...
        add(geo: Geometry): void;
        add(collection: JstsCollection<Geometry>): void;

        getGeometry(): Geometry;

        getCutEdges(): JstsCollection<Geometry>;
        getDangles(): JstsCollection<Geometry>;
        getInvalidRingLines(): JstsCollection<Geometry>;
        getPolygons(): JstsCollection<Geometry>;
    ...
    namespace valid {
      export class TopologyValidationError {
        constructor(errorType: number, pt?: Coordinate);
        getErrorType: number;
        getMessage(): string;
        toString(): string;
      }
  ...
  namespace shape {
    export class CubicBezierCurve {
      constructor(geom: geom.Geometry, alpha: number, skew: number);
